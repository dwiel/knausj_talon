...
change_screen_mode = mod.setting(
    "window_manager_change_screen_mode", type=str, default="same"
)
...
def move_screen(off=None, screen_number=None, win=None):
    ...
    if current_change_screen_mode == "same":
        ...
        raise ValueError(
            "{} screen mode not understood." % (current_change_screen_mode)
        )

    print("dst", dst)
    print("new_rectangle", new_rectangle)
    # return
...
def resize_window(x, y, w, h):
    ...
    # print("screen dimensions:", rect.x, rect.y, rect.width, rect.height)
    rect.x += rect.width * x
    rect.y += rect.height * y
    rect.width *= w
    rect.height *= h
    print("new window dimensions:", rect)
    win.rect = rect
